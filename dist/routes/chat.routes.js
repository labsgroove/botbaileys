...
const router = Router();
// LISTAR CHATS
router.get("/session/:id/chats", authenticateToken, async (req, res) => {
    const { id } = req.params;
    if (typeof id !== 'string') {
    ...
    for (const chat of dbChats)
        merged.set(chat.jid, chat);
    for (const chat of memoryChats) {
        const existing = merged.get(chat.jid) || {};
        merged.set(chat.jid, { ...existing, ...chat });
    }
    const chats = Array.from(merged.values()).sort((a, b) => (b.lastTimestamp || 0) - (a.lastTimestamp || 0));
    return res.json({ chats });
});
// INFORMACOES DO CONTATO
router.get("/session/:id/contacts/:jid", authenticateToken, async (req, res) => {
...
router.get("/session/:id/messages/:jid", async (req, res) => {
    ...
    try {
        ...
        for (const message of memoryMessages)
            merged.set(message.id, message);
        const messages = Array.from(merged.values()).sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
