...
export class MessageSender {
    ...
    async sendReaction(payload) {
        ...
        const targetMessage = ChatStore.getMessage(this.sessionId, payload.jid, reaction.messageId);
        const targetKey = {
    ...
    async sendListMessage(payload) {
        ...
        ChatStore.addOutgoing(this.sessionId, {
            ...
            interactive: {
                ...
                footer: interactive.footer,
                options: (interactive.sections || []).flatMap((section) => (section.rows || []).map((row) => ({
                    id: row.id,
                    title: row.title,
                    description: row.description,
                }))),
    ...
    async sendButtonsMessage(payload) {
        const interactive = payload.interactive || {};
    ...
    async sendMedia(payload) {
        ...
        const content = this.buildMediaContent(mediaKind, normalized.source, normalized.mimetype, caption, payload);
    ...
    buildMediaSource(payload) {
        ...
        if (!mediaUrl) {
            throw new Error("Mídia não informada. Envie mediaDataUrl (base64) ou mediaUrl");
        }
        return {
            source: { url: mediaUrl },
    ...
    async normalizeAudioVoiceNote(args) {
    ...
    buildMediaContent(kind, source, mimetype, caption, payload) {
        const content = {};
        if (kind === "image") {
    ...
    getMediaText(kind, caption, fileName) {
        const mediaTexts = {
...
async function convertAudioToOggOpus(input) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        ffmpeg.on("error", (error) => {
            clearTimeout(timer);
            reject(new Error(`Falha ao iniciar ffmpeg para converter áudio: ${error.message}`));
        });
        ffmpeg.on("close", (code) => {
            ...
            reject(new Error(`Falha ao converter áudio para OGG/Opus (ffmpeg exit ${code}).${stderr ? ` ${stderr}` : ""}`));
        });
