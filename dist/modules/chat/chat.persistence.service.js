...
import { messagePreview, normalizeJid, normalizeMessageStatus, pickContactName, } from "../../utils/message.utils";
...
function chooseCanonicalJid(candidates) {
...
function buildMessageDocId(sessionId, messageId) {
...
export class ChatPersistenceService {
    ...
    static persistMessage(sessionId, message, options = {}) {
        if (!shouldPersistMessage(message)) {
            return;
        }
        const normalizedRemoteJid = normalizeJid(message.jid) || message.jid;
        const resolvedJid = ChatStore.resolveChatJid(sessionId, normalizedRemoteJid) ||
            normalizedRemoteJid;
        ...
        const normalizedStatus = message.status
            ...
            : undefined;
        const persistedType = message.isDeleted
            ? "system"
        ...
        this.enqueue(sessionId, async () => {
            ...
            await ChatThread.updateOne({ sessionId, jid: resolvedJid }, {
                $setOnInsert: {
                    sessionId,
                    jid: resolvedJid,
                    unread: 0,
            ...
            if (shouldCountUnread) {
                await ChatThread.updateOne({ sessionId, jid: resolvedJid }, { $inc: { unread: shouldCountUnread } });
            }
            await ChatThread.updateOne({ sessionId, jid: resolvedJid, lastTimestamp: { $lte: timestamp } }, {
                $set: {
            ...
            await CrmContact.updateOne({ sessionId, jid: resolvedJid }, {
                $setOnInsert: {
                    sessionId,
                    jid: resolvedJid,
                    kind,
    ...
    static persistHistoryChat(sessionId, payload) {
        const normalizedJid = normalizeJid(payload.jid) || payload.jid;
        const resolvedJid = ChatStore.resolveChatJid(sessionId, normalizedJid) || normalizedJid;
    ...
    static persistContactUpdate(sessionId, contact) {
        ...
        const aliasSet = Array.from(new Set([...normalizedCandidates, ...resolvedCandidates]));
        const canonical = chooseCanonicalJid(resolvedCandidates.length ? resolvedCandidates : normalizedCandidates) || normalizedCandidates[0];
        ...
        this.enqueue(sessionId, async () => {
            ...
            if (!existing) {
                ...
                if (existingCanonical !== canonical) {
                    await CrmContact.updateOne({ sessionId, jid: existingCanonical }, { $set: { jid: canonical } });
                }
                await CrmContact.updateOne({ sessionId, jid: canonical }, {
                    $set: {
                        kind,
                        ...(phone ? { phone } : {}),
    ...
    static persistMessageUpdate(sessionId, payload) {
        const normalizedJid = normalizeJid(payload.jid) || payload.jid;
        const resolvedJid = ChatStore.resolveChatJid(sessionId, normalizedJid) || normalizedJid;
    ...
    static async resolveChatJid(sessionId, jid) {
    ...
    static async listChats(sessionId) {
        ...
        return threads.map((thread) => ({
            ...
            lastMessageType: thread.lastMessageType || undefined,
        }));
    }
    static async getMessages(sessionId, jid, options = {}) {
        if (!database.isConnectionActive()) {
            return [];
        }
    ...
    static markAsRead(sessionId, jid) {
        const normalized = normalizeJid(jid) || jid;
        const resolved = ChatStore.resolveChatJid(sessionId, normalized) || normalized;
        this.enqueue(sessionId, async () => {
            await ChatThread.updateOne({ sessionId, jid: resolved }, { $set: { unread: 0 } }, { upsert: false });
        });
    }
    static async mergeCanonical(sessionId, canonicalJid, aliases) {
        const aliasSet = Array.from(new Set([canonicalJid, ...aliases]));
        await ChatMessage.updateMany({ sessionId, jid: { $in: aliasSet } }, { $set: { jid: canonicalJid } });
        ...
        let bestLastMessageType = target.lastMessageType;
        ...
        for (const thread of threads) {
            ...
            if (threadTimestamp > bestLastTimestamp) {
                ...
                bestLastMessageType = thread.lastMessageType || bestLastMessageType;
            }
        }
        ...
        if (bestLastMessageType) {
            target.lastMessageType = bestLastMessageType;
        }
        ...
        const mergedContactAliases = new Set([
            canonicalJid,
            ...aliasSet,
...
ChatPersistenceService.queues = new Map();
