import { normalizeJid as normalizeJidUtil, normalizeMessageStatus, messagePreview } from "../../utils/message.utils";
const store = {};
...
function resolveJid(session, jid) {
...
function ensureChatByCanonical(session, jid) {
...
function ensureChat(session, jid) {
...
function hasChat(session, jid) {
    return !!session.messages[jid] || !!session.meta[jid];
}
function chooseCanonicalJid(session, candidates) {
...
function choosePreferredJid(session, firstJid, secondJid) {
    const firstPriority = jidPriority(firstJid);
    const secondPriority = jidPriority(secondJid);
    if (firstPriority !== secondPriority) {
...
function mergeChats(session, targetJid, sourceJid) {
...
function nextMessageId(jid, timestamp) {
...
function shouldCreateChatForMessage(message) {
...
function upsertMessage(sessionId, payload, options = {}) {
    // Valida se a mensagem deve criar um chat
    const messageObj = {
        ...payload,
        timestamp: payload.timestamp || Date.now(),
        status: payload.status ? normalizeMessageStatus(payload.status) : undefined,
    };
...
function findMessage(session, jid, messageId) {
...
function addSessionEvent(session, name, summary) {
...
export class ChatStore {
    static addEvent(sessionId, name, summary) {
        const session = ensureSession(sessionId);
        addSessionEvent(session, name, summary);
    }
    static listEvents(sessionId, limit = 80) {
        const session = ensureSession(sessionId);
    ...
    static upsertContact(sessionId, payload) {
        const session = ensureSession(sessionId);
        // Coleta todos os JIDs candidatos e normaliza
        const rawCandidates = [payload.jid, payload.id, payload.lid].filter(Boolean);
        ...
        const resolvedCandidates = Array.from(new Set(normalizedCandidates
    ...
    static addIncoming(sessionId, payload) {
        upsertMessage(sessionId, {
            ...payload,
            fromMe: false,
        }, { countUnread: true });
    }
    static addOutgoing(sessionId, payload) {
        upsertMessage(sessionId, {
            ...payload,
            fromMe: true,
        }, { countUnread: false });
    }
    static addHistory(sessionId, payload) {
        // Usa a mesma função upsertMessage para histórico, garantindo consistência
        upsertMessage(sessionId, payload, { countUnread: false });
    }
    static updateMessage(sessionId, payload) {
        const session = ensureSession(sessionId);
    ...
    static applyReaction(sessionId, payload) {
        const session = ensureSession(sessionId);
    ...
    static markMessageDeleted(sessionId, jid, messageId, timestamp = Date.now()) {
    ...
    static upsertHistoryChat(sessionId, payload) {
        const session = ensureSession(sessionId);
        const chat = ensureChat(session, payload.jid);
        if (!chat) {
            return;
    ...
    static resolveChatJid(sessionId, jid) {
        const session = ensureSession(sessionId);
        return resolveJid(session, jid);
    }
    static listChats(sessionId) {
        const session = ensureSession(sessionId);
        return Object.values(session.meta)
            .map((chat) => ({ ...chat }))
            .sort((a, b) => b.lastTimestamp - a.lastTimestamp);
    }
    static getMessage(sessionId, jid, messageId) {
        const session = ensureSession(sessionId);
        const message = findMessage(session, jid, messageId);
        return message ? { ...message } : undefined;
    }
    static getMessages(sessionId, jid) {
        const session = ensureSession(sessionId);
        const chat = ensureChat(session, jid);
        if (!chat) {
            return [];
    ...
    static setProfilePictureUrl(sessionId, jid, profilePictureUrl) {
        const session = ensureSession(sessionId);
        const chat = ensureChat(session, jid);
        if (!chat) {
            return;
    ...
    static getProfilePictureUrl(sessionId, jid) {
        const session = ensureSession(sessionId);
    ...
    static updateProfilePictureUrl(sessionId, jid, profilePictureUrl) {
        const session = ensureSession(sessionId);
        const chat = ensureChat(session, jid);
        if (!chat) {
            return;
