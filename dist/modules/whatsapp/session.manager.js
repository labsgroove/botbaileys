...
import { normalizeTimestamp, normalizeJid, pickContactName } from "../../utils/message.utils";
...
globalThis.contactNamesBySession = contactNamesBySession;
...
function setContactName(sessionId, jid, name) {
...
function emitContactUpdate(options, payload) {
    const contact = {
    ...
    const hasNameUpdate = !!(contact.name ||
        contact.notify ||
...
function emitChatSnapshot(options, payload, sessionId) {
...
function emitEvent(options, name, summary) {
    options.onEvent?.({ name, summary });
}
export async function createSession(sessionId, options = {}) {
    ...
    sock.ev.on("messages.update", (updates) => {
        ...
        for (const item of updates || []) {
            ...
            options.onMessageUpdate?.({
                ...
                text: parsed.text,
                timestamp: parsed.timestamp,
                status: item.update?.status !== undefined
    ...
    sock.ev.on("message-receipt.update", (events) => {
        ...
        for (const receiptEvent of events || []) {
            ...
            options.onMessageReceipt?.({
                ...
                timestamp: normalizeTimestamp(receiptEvent?.receipt?.playedTimestamp) ||
    ...
    sock.ev.on("presence.update", (event) => {
        ...
        for (const [participant, presence] of participants) {
            options.onPresenceUpdate?.({
                ...
                lastKnownPresence: presence?.lastKnownPresence || undefined,
            });
        }
    });
    sock.ev.on("messaging-history.set", ({ messages, contacts, chats }) => {
        ...
        emitEvent(options, "messaging-history.set", `Historico: mensagens=${totalHistoryMessages}, contatos=${contacts?.length || 0}, chats=${chats?.length || 0}`);
        ...
        console.log(`[${sessionId}] Historico recebido: total=${totalHistoryMessages}, importadas=${importedMessages}, ignoradas=${skippedMessages}, contatos=${contacts?.length || 0}`);
    });
    ...
    sock.ev.on("group-participants.update", (event) => {
        emitEvent(options, "group-participants.update", `Grupo ${event.id}: ${event.action} (${event.participants?.length || 0} participante(s))`);
    });
