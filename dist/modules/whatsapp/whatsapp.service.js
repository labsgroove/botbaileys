...
function setState(sessionId, next) {
...
function buildMediaSource(payload) {
...
function inferMediaType(payload) {
...
function isMediaExpired(message) {
...
function extractMediaKey(message) {
    if (!message?.message)
        return null;
    const mediaTypes = [
        'imageMessage',
...
function cacheRawMessage(sessionId, jid, id, raw) {
    if (!jid || !id || !raw) {
        return;
    }
...
function findRawMessage(sessionId, jid, messageId) {
    const cache = rawMessages.get(sessionId);
    if (!cache) {
        return undefined;
    }
...
export class WhatsAppService {
    static async initSession(sessionId, force = false) {
        ...
        const sock = await createSession(sessionId, {
            ...
            onPresenceUpdate: ({ jid, participant, lastKnownPresence }) => {
                ChatStore.addEvent(sessionId, "presence.update", `${participant} em ${jid}: ${lastKnownPresence || "desconhecido"}`);
            },
    ...
    static async sendMessage(sessionId, payload) {
    ...
    static async getMediaContent(sessionId, jid, messageId) {
        const sock = sessions.get(sessionId);
        if (!sock) {
            throw new Error("Sessao nao encontrada");
        }
    ...
    static async deleteSessionCredentials(sessionId) {
        try {
    ...
    static async closeSession(sessionId, deleteCredentials = false) {
        const sock = sessions.get(sessionId);
        if (sock) {
            await sock.logout();
            sessions.delete(sessionId);
